...
export default function ContactsPage() {
  ...
  const fetcher = useCallback(
    () => adminApi.listContacts({ status: status || undefined, limit: LIMIT, offset }),
    [status, offset]
  );
  ...
  return (
    <PageBody title="Contacts">
      ...
      {query.status === "success" && query.data.items.length === 0 && (
        <EmptyState title="No contacts found" hint="Try adjusting the selected filters." />
      )}
      {query.status === "success" && query.data.items.length > 0 && (
        <>
            ...
            <table className={tableStyles.table}>
              <thead>
                <tr>
                  <th>Phone</th>
                  <th>Status</th>
                  <th className={tableStyles.numeric}>Attempts</th>
                  <th>Suppressed</th>
                  <th>Added</th>
                </tr>
              </thead>
              <tbody>
                {query.data.items.map((c) => (
                  <tr key={c.id}>
                    ...
                    <td className={tableStyles.numeric}>{c.attempt_count}</td>
                    <td>{c.suppressed ? <StatusDot label="Suppressed" tone="danger" /> : "—"}</td>
